...
describe('useFavicon', () => {
  ...
  it('should set the elements href to the provided string', () => {
    renderHook(() => useFavicon('https://github.com/SamerKK0/use-reacts-hooks'));
    const favicon = document.querySelector("link[rel*='icon']") as HTMLLinkElement;

    expect(favicon.href).toBe('https://github.com/SamerKK0/use-reacts-hooks');
  });

  it('should update an existing favicon', () => {
    const hook = renderHook((props) => useFavicon(props), {
      initialProps: 'https://github.com/SamerKK0/use-reacts-hooks',
    });
    const favicon = document.querySelector("link[rel*='icon']") as HTMLLinkElement;
